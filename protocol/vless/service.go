...
func (s *Service[T]) NewConnection(ctx context.Context, conn net.Conn, source M.Socksaddr, onClose N.CloseHandlerFunc) error {
	...
	user, err := uuid.FromBytes(request.UUID[:])
	if err != nil {
		return E.New("unknown UUID: ", uuid.FromBytesOrNil(request.UUID[:]))
	}
	ctx = auth.ContextWithUser(ctx, user.String())
	userFlow := request.Flow
	if request.Flow == vless.FlowVision && request.Command == vmess.NetworkUDP {
		return E.New(vless.FlowVision, " flow does not support UDP")
	...
	switch userFlow {
	case vless.FlowVision:
		conn, err = vless.NewVisionConn(responseConn, conn, request.UUID, s.logger)
		if err != nil {
...
func (c *serverConn) Write(b []byte) (n int, err error) {
	if !c.responseWritten {
		_, err = bufio.WriteVectorised(c.writer, [][]byte{{vless.Version, 0}, b})
		if err == nil {
			n = len(b)
		}
...
func (c *serverConn) WriteVectorised(buffers []*buf.Buffer) error {
	if !c.responseWritten {
		err := c.writer.WriteVectorised(append([]*buf.Buffer{buf.As([]byte{vless.Version, 0})}, buffers...))
		c.responseWritten = true
		return err
	}
...
func (c *serverPacketConn) WriteTo(p []byte, addr net.Addr) (n int, err error) {
	if !c.responseWritten {
		if c.responseWriter == nil {
			...
			_, err = bufio.WriteVectorised(bufio.NewVectorisedWriter(c.ExtendedConn), [][]byte{{vless.Version, 0}, packetLen[:], p})
			if err == nil {
				n = len(p)
			}
			c.responseWritten = true
			...
			_, err = c.responseWriter.Write([]byte{vless.Version, 0})
			if err != nil {
				return
			}
			c.responseWritten = true
...
func (c *serverPacketConn) WritePacket(buffer *buf.Buffer, destination M.Socksaddr) error {
	if !c.responseWritten {
		if c.responseWriter == nil {
			...
			err := bufio.NewVectorisedWriter(c.ExtendedConn).WriteVectorised([]*buf.Buffer{buf.As([]byte{vless.Version, 0}), buf.As(packetLen[:]), buffer})
			c.responseWritten = true
			return err
		} else {
			_, err := c.responseWriter.Write([]byte{vless.Version, 0})
